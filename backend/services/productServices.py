...
def Fk_delete_check(category_id: int, subcategory_id: int):
    ...
    subcategory_vo_list = subcategory_dao.readSubCategory(subcategory_id)
...
def adminInsertProduct(category_id: int, subcategory_id: int, image_filename: str,
    ...
    try:
        ...
        if category_vo_list is None or subcategory_vo_list is None:
            ...
            product_dao.createProduct(product_data=product_vo)
            return product_vo
...
def adminReadProducts():
    try:
        category_dao = CategoryDAO()
        subcategory_dao = SubCategoryDAO()
        product_dao = ProductDAO()

        product_data = product_dao.readProducts()
        if product_data:
            ...
            data_to_show = [
                ...
                if not category_dao.readCategory(product.product_category_id) is None
                and not subcategory_dao.readSubCategory(product.product_subcategory_id) is None
            ]
...
def adminDeleteProduct(product_id: int):
    try:
        product_dao = ProductDAO()
        product_vo_list = product_dao.editProduct(product_id)

        if product_vo_list is not None and product_vo_list.is_deleted == 0:

            delete_check = Fk_delete_check(product_vo_list.product_category_id, product_vo_list.product_subcategory_id)
            if delete_check is None:
                ...
                product_dao.updateProduct(product_vo_list)
...
def adminUpdateProductData(product_update_id: int, product_update_data: UpdateProductDataDTO):
    try:
        product_dao = ProductDAO()
        product_vo_list = product_dao.editProduct(product_update_id)

        if product_vo_list is not None and product_vo_list.is_deleted == 0:
            delete_value = Fk_delete_check(product_vo_list.product_category_id, product_vo_list.product_subcategory_id)
            ...
            if delete_value is None:
                ...
                product_dao.updateProduct(product_vo_list=product_vo_list)
...
def adminUpdateProductImage(update_image_id: int, update_image_name: str,
                            product_image_data: bytes):
    try:
        product_dao = ProductDAO()

        product_vo_list = product_dao.editProduct(update_image_id)
